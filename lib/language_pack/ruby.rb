...
class LanguagePack::Ruby < LanguagePack::Base
  ...
  def build
    ...
    allow_git do
      ...
      gem_layer.metadata[:ruby_version] = run_stdout("ruby -v").chomp
      gem_layer.metadata[:rubygems_version] = run_stdout("gem -v").chomp
  ...
  def setup_export(layer = nil)
    instrument "ruby.setup_export" do
      ...
      paths = paths.map { |path| !/^\/.*/.match?(path) ? "#{build_path}/#{path}" : path }.join(":") unless layer
  ...
  def load_default_cache?
    false # new_app? && ruby_version.default?
  end
